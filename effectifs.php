<?php
	...
	try {
		$stmt = $pdo->query("SELECT id, name, surname, grade, service, active FROM user");
				...
				<?php if (!empty($users_list)) : ?>
					<?php foreach ($users_list as $user) : ?>
							<div class="effectif-box">
								<?php if ($_SESSION['user']['grade'] == 'D' || $_SESSION['user']['grade'] == 'DA' || $_SESSION['user']['grade'] == 'CS') {
											if ($user['active'] === 'Yes') {
												echo "<span>✅</span>";
											}
											else {
								...
								<div class="effectif-id">
									<a href="profile.php?id=<?= urlencode($user['id']); ?>" class="effectif-box-link"><h3><?= htmlspecialchars($user['name']) . ' ' . htmlspecialchars($user['surname']); ?></h3></a>
								</div>
								<div class="effectif-service">
									<span class="<?= $user['service'] === 'Yes' ? 'in-service' : 'out-of-service'; ?>">
										<?= $user['service'] === 'Yes' ? 'En service' : 'Pas en service'; ?>
									</span>
								</div>
